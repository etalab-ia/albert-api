...
with col2:
    with st.expander("Delete a collection", icon="📦"):
        collection = st.selectbox(
            ...
            [f"{collection["id"]} ({collection["name"]})" for collection in collections],
...
with col1:
    with st.expander("Upload a file", icon="📑"):
        collection = st.selectbox(
            "Select collection to delete", [f"{collection["id"]} ({collection["name"]})" for collection in collections], key="upload_file_selectbox"
        )
