...
async def get_model(
    request: Request, model: str = Path(description="The name of the model to get."), user: AuthenticatedUser = Security(RateLimit())
) -> Model:
    """
    Get a model by name.
    """
...
async def get_models(request: Request, user: AuthenticatedUser = Security(RateLimit())) -> Union[Models, Model]:
    """
