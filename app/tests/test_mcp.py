...
from app.utils.variables import ENDPOINT__CHAT_COMPLETIONS, ENDPOINT__COLLECTIONS, ENDPOINT__DOCUMENTS, ENDPOINT__FILES, \
...
class TestMCP:
    ...
    def test_chat_completions_unstreamed_response_tool_call(self, client: TestClient, setup):
    ...
    @respx.mock
    @responses.activate
    def test_chat_completions_unstreamed_response_tool_call_respx(self, client: TestClient, setup):
        params = {"model": "albert-large", "messages": [{"role": "user", "content": "Donne moi la météo à Miami"}],
                  "stream": False, "n": 1, "max_tokens": 1000}
        tools_response = {
            "weather": {
