...
class TestModels:
    def test_get_models_response_status_code(self, client: TestClient):
        ...
        response = client.get_without_permissions(url=f"/v1/models/{model}")
    ...
    def test_get_models_non_existing_model(self, client: TestClient):
        ...
        response = client.get_without_permissions(url="/v1/models/non-existing-model")
    ...
    def test_get_models_aliases(self, client: TestClient):
        ...
        response = client.get_without_permissions(url=f"/v1/models/{model.id}")
        assert response.json()["aliases"] == model.aliases

        response = client.get_without_permissions(url=f"/v1/models/{model.aliases[0]}")
