...
def setup(client: TestClient):
    ...
    # Create a collection
    response = client.post_without_permissions(url="/v1/collections", json={"name": f"test_collection_{uuid4()}"})
...
class TestSearch:
    def test_search(self, client: TestClient, setup):
        ...
        response = client.post_without_permissions(url="/v1/search", json=data)
        assert response.status_code == 200, response.text
