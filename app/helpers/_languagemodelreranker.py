...
class LanguageModelReranker:
    PROMPT_LLM_BASED = """Voilà un texte : {doc}\n 
...
Le texte n'a pas besoin de répondre parfaitement à la question, juste d'apporter des éléments de réponses et/ou de parler du même thème. Réponds uniquement 0 ou 1."""
    ...
    def create(self, prompt: str, inputs: list) -> List[Rerank]:
        data = list()
        for index, input in enumerate(inputs):
            content = self.PROMPT_LLM_BASED.format(prompt=prompt, doc=input)

            response = self.model.chat.completions.create(
                messages=[{"role": "user", "content": content}], model=self.model, temperature=0.1, max_tokens=3, stream=False, n=1
            )
