...
from app.utils.exceptions import InsufficientRightsException, InvalidAPIKeyException, InvalidAuthenticationSchemeException, ModelNotFoundException
...
class Authorization:
    ...
    async def __call__(self, request: Request, api_key: Annotated[HTTPAuthorizationCredentials, Depends(dependency=HTTPBearer(scheme_name="API key"))]) -> User:  # fmt: off
        ...
        # limits
        if request.url.path.endswith(ENDPOINT__CHAT_COMPLETIONS) and request.method == "POST":  # @TODO: extend to other models endpoints
            try:
        ...
        if request.url.path.endswith(ENDPOINT__COLLECTIONS) and request.method == "DELETE":
            body = await request.body()
            body = json.loads(body) if body else {}
            if body["type"] == COLLECTION_TYPE__PUBLIC and PermissionType.DELETE_PUBLIC_COLLECTION not in user.permissions:
                raise InsufficientRightsException()
        ...
        # TODO change
        if request.url.path.endswith(ENDPOINT__DOCUMENTS) and request.method == "DELETE":
            body = await request.body()
            body = json.loads(body) if body else {}
            if body["type"] == COLLECTION_TYPE__PUBLIC and PermissionType.DELETE_PUBLIC_COLLECTION not in user.permissions:
                raise InsufficientRightsException()
