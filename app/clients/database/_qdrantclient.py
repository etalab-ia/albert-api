...
class QdrantClient(AsyncQdrantClient):
    def __init__(self, *args, **kwargs):
        ...
        assert super().ping(), "Qdrant is not reachable"

    async def create_collection(self, collection_id: int, vector_size: int) -> None:
        await self.super().create_collection(
    ...
    async def delete_collection(self, collection_id: int) -> None:
        await self.super().delete_collection(collection_name=str(collection_id))

    async def get_chunk_count(self, collection_id: int, document_id: int) -> Optional[int]:
        try:
            chunks_count = await self.super().count(
                collection_name=str(collection_id),
    ...
    async def delete_document(self, collection_id: int, document_id: int) -> None:
        ...
        await self.super().delete(collection_name=str(collection_id), points_selector=FilterSelector(filter=filter))
    ...
    async def search(
        ...
        if method == SearchMethod.SEMANTIC:
            searches = await self._semantic_query(prompt=prompt, collection_ids=collection_ids, size=k)

        elif method == SearchMethod.LEXICAL:
            searches = await self._lexical_search(query_vector=query_vector, collection_ids=collection_ids, k=k, score_threshold=score_threshold)  # fmt: off

        else:  # method == SearchMethod.HYBRID
            searches = await self._hybrid_search(prompt=prompt, query_vector=query_vector, collection_ids=collection_ids, k=k, rff_k=rff_k)
    ...
    async def _semantic_query(self, query_vector: list[float], collection_ids: List[int], k: int, score_threshold: float = 0.0) -> List[Search]:
        ...
        for collection_id in collection_ids:
            results = await self.super().search(
                collection_name=str(collection_id),
    ...
    async def _hybrid_search(self, prompt: str, embedding: list[float], collection_ids: List[int], k: int, rff_k: Optional[int] = 20) -> List[Search]:
        raise NotImplementedException("Only semantic search is available for Qdrant database.")
